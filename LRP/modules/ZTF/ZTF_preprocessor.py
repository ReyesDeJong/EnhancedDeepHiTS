...
class ZTF_data_preprocessor(object):
    ...
    def check_samples_nans(self, samples):
        nans_sample_idx = []
        for i in range(len(samples)):
            sample = samples[i]
    ...
    def clean_nans(self, samples):
        nans_sample_idx = self.check_samples_nans(samples)
...
if __name__ == "__main__":
    ...
    data_processor.save_to_pickle(path_data, array2save = preprocessed_reals, file_name="broker_reals")
    data_processor.save_to_pickle(path_data, array2save = preprocessed_bogus, file_name="broker_bogus")
